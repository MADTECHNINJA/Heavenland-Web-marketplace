...
import { AuctionHouse } from './metaplex/auctionHouse';
...
export class Metaplex {
    ...
    constructor() {
        ...
        this.auctionHouse = new AuctionHouse(this.connection);
    }
    ...
    private _confirmTransaction = async (signature: string) => {
        return await this.connection.value.confirmTransaction(signature, 'finalized');
    };
    ...
    retrieveRequestItems = async (items: string[]) => {
        ...
        for (const i of items) {
            const metadata = await programs.metadata.Metadata.findByMint(this.connection.value, new web3.PublicKey(i));
        ...
        for (const metadata of onChain) {
            requests.push(axios.get(API.NftService.getData(metadata.mint)));
        }
    ...
    getTPS = async () => {
        const samples = await this.connection.value.getRecentPerformanceSamples(5);

        return samples && samples[0] ? samples[0].numTransactions / samples[0].samplePeriodSecs : null;
    };

    getLatestBlockhash = async () => {
        return (await this.connection.value.getLatestBlockhash('finalized'))?.blockhash;
    };
    ...
    sendRawTransaction = async (tx: web3.Transaction, onSent?: (txSig) => void) => {
        const txSig = await this.connection.value.sendRawTransaction(tx.serialize());
    ...
    sendTransaction = async (tx: web3.Transaction, onSent?: (txSig) => void) => {
        ...
        const txSig = await sendTransaction(tx, this.connection.value);
    ...
    fetchTokens = async (publicKey: web3.PublicKey | null) => {
        ...
        let onChain: OnChainMetadata[] = await programs.metadata.Metadata.findDataByOwner(
            this.connection.value,
            publicKey
        );

        const updateAuthorities = CollectionFactory.getUpdateAuthorities();
    ...
    getBalanceSOL = async (wallet: web3.PublicKey | null) => {
        ...
        return await this.connection.value.getBalance(wallet);
    };
    ...
    getBalanceHTO = async (wallet: web3.PublicKey | null) => {
    ...
    #getCustomTokenBalance = async (tokenAddress: string, wallet: web3.PublicKey | null) => {
        ...
        const res = await this.connection.value.getParsedTokenAccountsByOwner(wallet, {
        ...
        try {
            await axios.all(requests).then((responses) => {
                for (const index in responses) {
                    const responseData = responses[index].data?.json;
    ...
    async #getATA(mintAddress: string, wallet: web3.PublicKey) {
        if (!this.connection.value || !wallet) {
            return null;
        }
        ...
        let sourceAta;
        const sourceTokenAccounts = await this.connection.value.getParsedTokenAccountsByOwner(wallet, {
    ...
    fetchStakedTokens = async (mints: web3.PublicKey[]) => {
        if (!this.connection.value) return null;

        const onChain: OnChainMetadata[] = [];
        const response = [];
        mints.map((mint) =>
            response.push(
                programs.metadata.Metadata.findByMint(this.connection.value, mint).then((res) => {
    ...
    fetchStakingFromAddress = async (treasuryAddress: string, htoAccount: string, rewardVaults: Array<string>) => {
        if (!this.connection.value) {
            throw new Error('not defined: connection');
        } else if (!treasuryAddress) {
            throw new Error('not defined: treasuryAddress');
        } else if (!htoAccount) {
            throw new Error('not defined: htoAccount');
        }

        const res = await this.connection.value.getParsedTokenAccountsByOwner(new web3.PublicKey(treasuryAddress), {
            programId: TOKEN_PROGRAM_ID,
        });

        const tokens = res.value.filter(
            (a) =>
                a.pubkey?.toBase58() !== htoAccount &&
                !rewardVaults.includes(a.pubkey?.toBase58()) &&
                a.account?.data?.parsed?.info?.tokenAmount?.uiAmount === 1
        );

        const htoTreasuryAddress = res.value.find((a) => a.pubkey?.toBase58() === htoAccount);
        ...
        return {
            ...
            htoAmount: htoTreasuryAddress.account?.data?.parsed?.info?.tokenAmount?.amount,
        };
    };

    getTokenOwner = async (mintId: string) => {
        const largestAccounts = await this.connection.value.getTokenLargestAccounts(new web3.PublicKey(mintId));
        const largestAccountInfo = await this.connection.value.getParsedAccountInfo(largestAccounts.value[0].address);
