...
export const createMergeStakes = async (
    mint: PublicKey,
    userAddress: PublicKey,
    program: anchor.Program,
    connection: Connection
    ...
    logger.info('[STAKING] createMergeStakes: globalAuthority', globalAuthority.toBase58());
    const userPoolKey = await PublicKey.createWithSeed(userAddress, 'user-pool', STAKING_PROGRAM_ID);
    const ret1 = await getATokenAccountsNeedCreate(connection, userAddress, userAddress, [HTO_TOKEN_MINT]);
    const ret2 = await getATokenAccountsNeedCreate(connection, userAddress, globalAuthority, [HTO_TOKEN_MINT]);
    logger.info('[STAKING] createMergeStakes: Dest HTO Account', ret1.destinationAccounts[0].toBase58());
    logger.info('[STAKING] createMergeStakes: Src HTO Account', ret2.destinationAccounts[0].toBase58());
    ...
    logger.info('[STAKING] createMergeStakes: merging', userAddress.toBase58());
    tx.add(
        program.instruction.mergeStakes(bump, nft_bump, {
            accounts: {
                owner: userAddress,
                userPool: userPoolKey,
                globalAuthority,
                ...
                rewardVault: ret2.destinationAccounts[0],
                tokenProgram: TOKEN_PROGRAM_ID,
            },
            instructions: [],
            signers: [],
...
export const createClaimRewardTx = async (
    ...
    logger.info('[STAKING] createClaimRewardTx', userAddress?.toBase58());
    ...
    logger.info('[STAKING] createClaimRewardTx: User', userAddress.toBase58());
