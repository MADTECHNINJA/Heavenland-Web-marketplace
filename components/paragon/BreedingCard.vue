<template>
    ...
    >
        ...
        <div class="px-3 grow py-4 bg-indigo-930 rounded-b-lg flex items-center flex-col justify-end">
            <div class="flex items-center space-x-1.5 mb-1">
                <FontAwesomeIcon :icon="['fas', 'split']" class="h-3 w-3 mr-0.5 text-white" />
            ...
            <div class="mb-3 mt-2 px-3 py-2 border w-full border-gray-400 border-opacity-20 rounded-md text-xs">
                <p class="text-gray-200 text-center font-bold mb-2">First paragon mint</p>

                <div class="flex justify-center">
                    <AppMintSolscanLink :mint="operation.nftMint2.toString()" />
                    <p class="font-normal text-gray-300">
                        {{ formatAddress(operation.nftMint1.toString()) }}
                    </p>
                </div>
            </div>

            <div class="mb-6 px-3 py-2 border w-full border-gray-400 border-opacity-20 rounded-md text-xs">
                <p class="text-gray-200 text-center font-bold mb-2">Second paragon mint</p>
                <div class="flex justify-center">
                    <AppMintSolscanLink :mint="operation.nftMint2.toString()" />
                    <p class="font-normal text-gray-300">
                        {{ formatAddress(operation.nftMint2.toString()) }}
                    </p>
                </div>
            </div>
...
<script lang="ts" setup>
    import { Breeding, OperationState } from '~/types/Paragon.types';
    ...
    const fetchOperationItem = async () => {
        if (props.operation) {
            try {
                const response = await Metaplex.getInstance().retrieveRequestItems([props.operation.bornMint]);
    ...
    const claim = async () => {
            ...
            message: {
                ...
                data: {
                    items: [
                        {
                            entity: 'Paragon',
                            value: props.operation.nftMint1,
                            type: MessageParamType.SOLSCAN,
                            renderFn: formatAddress,
                        },
                        {
                            entity: 'Paragon',
                            value: props.operation.nftMint2,
                            type: MessageParamType.SOLSCAN,
                            renderFn: formatAddress,
                        },
                        {
        ...
        try {
            const tx = await Paragon.getInstance().claimBreed(props.operation.pda);
