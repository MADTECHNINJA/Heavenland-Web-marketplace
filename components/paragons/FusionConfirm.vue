    ...
    <div class="flex flex-col md:flex-row items-start justify-between px-6 py-3">
        <div>
            <div class="flex flex-col space-x-1 mt-2 text-sm text-gray-300 max-w-xl leading-5 mr-2">
                <p>
                    By confirming this fusing, your Paragon NFT & {{ getParagonFusePrice(tier) }} HTO will be sent from
                    your wallet. Once it's confirmed & stamped, you'll receive this Paragon NFT back.
                </p>
            </div>
        </div>
        ...
        <div class="flex justify-between w-full mt-4 md:mt-0 md:w-auto flex-row items-center md:space-x-9">
            ...
            <AppButton :disabled="!allowToBuy" @click="fuse" :loading="loading">Fuse...</AppButton>
        </div>
    </div>
        ...
        <div class="flex flex-row md:flex-col space-x-2 md:space-x-0 md:space-y-2">
            ...
            >
                ...
                <p class="flex items-center font-bold text-white text-sm">
                    <span class="mr-1">{{ getParagonFusePrice(tier) }}</span>
            ...
            >
                ...
                <p class="flex items-center font-bold text-white text-sm">
                    <span class="mr-1">{{ getParagonFuseDuration(tier) }} days</span>
                </p>
            </div>
        </div>

        <CashbackOperationOverview :amount="getParagonFusePrice(tier)" class="max-w-xs"/>
    </div>

    <div class="flex flex-col space-x-1 mt-2 text-sm text-gray-300 max-w-xl leading-5 px-6 mb-6">
        <p>If the operation fails, you'll be returned your Paragon NFTs & {{ getParagonFusePrice(tier) }} HTO.</p>
    </div>
</template>

<script lang="ts" setup>
    import htoCurrency from '@/assets/currency/hto.png';
    import { getParagonFusePrice, getParagonFuseDuration } from '~/types/Paragon.utils';
    ...
    const allowToBuy = computed(() => {
        return balanceStore.balanceHto.getUnformattedAmount() - getParagonFusePrice(props.tier) > 0;
    });
