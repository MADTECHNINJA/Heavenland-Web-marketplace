<template>
    <div
        :class="!transparent ? ' shadow rounded-lg from-indigo-930 bg-gradient-to-r to-indigo-940' : 'rounded-b-lg'"
        class="px-6 py-6  flex justify-center md:justify-start  flex md:space-x-5"
    >
        <AppSpinner class="mr-2 md:mr-0" v-if="type === AlertType.LOADING" :size="6" />
        <FontAwesomeIcon
            v-else-if="icon"
        ...
        <div class="flex flex-col items-center md:items-start">
            ...
            <p class="text-sm mt-1" :class="[type === AlertType.ERROR ? 'text-red-500' : 'text-gray-300']">
                <slot name="desc" />
            </p>
        </div>
    </div>
