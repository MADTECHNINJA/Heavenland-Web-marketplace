...
export const filterTokens = (collectionId: CollectionName, tokens: ITokenMetadata[]) => {
    ...
    if (!updateAuthorities) {
        logger.warn('[ACCOUNT TOKENS] no updateAuthority specified for collection: ' + collectionId);
        return;
    }
...
export const sortTokens = (collectionId: CollectionName, tokens: INftBase[], listed: APIResponse<INftBase[]>): any => {
    if (!tokens.length) {
        return tokens;
    }
    ...
    switch (collectionId) {
        case CollectionName.PARCELS:
            return (tokens as Parcel[]).sort((a, b) => {
                const byListing = _sortTokensByListing(a, b, listed);
                if (byListing === 0) {
                    const byZone = _sortTokensByZone(a, b);
                    if (byZone === 0) {
                        return _sortTokensByCluster(a, b);
                    } else {
                        return byZone;
                    }
                } else {
        ...
        default:
            return (tokens as any[]).sort((a, b) => {
                return _sortTokensByListing(a, b, listed);
            });
    }
};
